...
if (isset($_POST['name']) && isset($_POST['place']) && isset($_POST['start_date']) && isset($_POST['end_date'])
    ...
    $imageName = "britto" . time();

    $start_date = $db->formatDate($start_date);
    $end_date = $db->formatDate($end_date);
    // create a new event
    $event = $db->storeEvent($name, $place, $start_date, $end_date, $capacity, $budget, $moderator, $contact, $details);
    
    if ($event) {
        // event stored successfully
        $event_id = $event["id"];
        $image = $db->uploadImage($event_id, $imageName, $image);    
